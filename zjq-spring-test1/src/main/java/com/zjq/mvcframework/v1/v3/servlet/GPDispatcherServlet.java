package com.zjq.mvcframework.v1.v3.servlet;
...
public class GPDispatcherServlet extends HttpServlet{
	...
	private void doInstance(){
		...
		try{
			for (String className : classNames) {
				...
				}else if(clazz.isAnnotationPresent(QService.class)){
					
					QService service = clazz.getAnnotation(QService.class);
	...
	private void doAutowired(){
		...
		for (Entry<String, Object> entry : ioc.entrySet()) {
			...
			for (Field field : fields) {
				...
				if(!field.isAnnotationPresent(QAutowired.class)){ continue; }

				QAutowired autowired = field.getAnnotation(QAutowired.class);
	...
	private void initHandlerMapping(){
		...
		for (Entry<String, Object> entry : ioc.entrySet()) {
			...
			if(!clazz.isAnnotationPresent(QController.class)){ continue; }
			...
			for (Method method : methods) {
				...
				if(!method.isAnnotationPresent(QRequestMapping.class)){ continue; }
		...
		private void putParamIndexMapping(Method method){
			...
			for (int i = 0; i < pa.length ; i ++) {
				for(Annotation a : pa[i]){
					if(a instanceof QRequestParam){
						String paramName = ((QRequestParam) a).value();
